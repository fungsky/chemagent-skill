...
本技能版**不依赖后端 LLM**：推理、分析、报告生成全部由 AI 助理完成，后端只提供
...
- 拿到数据后由 AI 助理自行分析、对比、撰写结论；合规初筛结果要注明「研发阶段参考，非官方判定」

## 智能问答（Q&A）

用户的问题不需要后端 LLM 也能得到完整回答：**AI 助理负责理解与推理，客户端负责取数**。
...
5. Analyze the returned data yourself (AI assistant) and format results with sources/confidence.
